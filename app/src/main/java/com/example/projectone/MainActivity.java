...
public class MainActivity extends AppCompatActivity implements VideoRVAdapter.VideoClickInterface{
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode==STORAGE_PERMISSION){
            if (grantResults.length>0 && grantResults[0]==PackageManager.PERMISSION_GRANTED){
                ...
                Toast.makeText(this, "Permission is Required so give Permission", Toast.LENGTH_SHORT).show();
                finish();
            }
        }
    }
    ...
    public void onVideoClick(int position) {
        Intent inext = new Intent(MainActivity.this,Video_Player_Activity.class);
        inext.putExtra("videoName",videoRVModelArrayList.get(position).getVideoName());
        inext.putExtra("videoPath",videoRVModelArrayList.get(position).getVideoPath());
        startActivity(inext);
